...
class AHRSInternal : public IIOCompleteNotification, public IBoardCapabilities {
    ...
    void SetAHRSPosData(AHRSProtocol::AHRSPosUpdate& ahrs_update, long sensor_timestamp)
    {
        ...
        ahrs->is_moving = (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_MOVING) != 0) ? true : false);
        ahrs->is_rotating = (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_YAW_STABLE) != 0) ? false : true);
        ahrs->altitude_valid = (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0) ? true : false);
        ahrs->is_magnetometer_calibrated
            = (((ahrs_update.cal_status & NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0) ? true : false);
        ahrs->magnetic_disturbance
            = (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0) ? true : false);
        ...
        for (int i = 0; i < MAX_NUM_CALLBACKS; i++) {
            ...
            if (callback != NULL) {
                ...
                callback->timestampedDataReceived(
                    system_timestamp, sensor_timestamp, ahrs_update, ahrs->callback_contexts[i]);
            }
        }
    ...
    void SetAHRSData(AHRSProtocol::AHRSUpdate& ahrs_update, long sensor_timestamp)
    {
        ...
        ahrs->UpdateDisplacement(
            ahrs->world_linear_accel_x, ahrs->world_linear_accel_y, ahrs->update_rate_hz, ahrs->is_moving);
 ...
 */
float AHRS::GetPitch() { return pitch; }
 ...
 */
float AHRS::GetRoll() { return roll; }
 ...
 */
float AHRS::GetCompassHeading() { return compass_heading; }
...
{
    return !((cal_status & NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) == NAVX_CAL_STATUS_IMU_CAL_COMPLETE);
}
 ...
 * from the sensor.
 */
bool AHRS::IsConnected() { return io->IsConnected(); }
 ...
 */
double AHRS::GetByteCount() { return io->GetByteCount(); }
 ...
 */
double AHRS::GetUpdateCount() { return io->GetUpdateCount(); }
 ...
 */
long AHRS::GetLastSensorTimestamp() { return this->last_sensor_timestamp; }
 ...
 */
float AHRS::GetWorldLinearAccelX() { return this->world_linear_accel_x; }
 ...
 */
float AHRS::GetWorldLinearAccelY() { return this->world_linear_accel_y; }
 ...
 */
float AHRS::GetWorldLinearAccelZ() { return this->world_linear_accel_z; }
 ...
 */
bool AHRS::IsMoving() { return is_moving; }
 ...
 * @return Returns true if the sensor is currently detecting motion.
 */
bool AHRS::IsRotating() { return is_rotating; }
 ...
 */
float AHRS::GetBarometricPressure() { return baro_pressure; }
 ...
 */
float AHRS::GetAltitude() { return altitude; }
 ...
 */
bool AHRS::IsAltitudeValid() { return this->altitude_valid; }
 ...
 */
float AHRS::GetFusedHeading() { return fused_heading; }
 ...
 */
bool AHRS::IsMagneticDisturbance() { return magnetic_disturbance; }
 ...
 */
bool AHRS::IsMagnetometerCalibrated() { return is_magnetometer_calibrated; }
 ...
 */
float AHRS::GetQuaternionW() { return quaternionW; }
/**
 ...
 */
float AHRS::GetQuaternionX() { return quaternionX; }
/**
 * Returns the real portion (X axis) of the Orientation Quaternion which
 * fully describes the current sensor orientation with respect to the
 * reference angle defined as the angle at which the yaw was last "zeroed".
 ...
 * @return Returns the real portion (X) of the quaternion.
 */
float AHRS::GetQuaternionY() { return quaternionY; }
/**
 * Returns the real portion (X axis) of the Orientation Quaternion which
 * fully describes the current sensor orientation with respect to the
 * reference angle defined as the angle at which the yaw was last "zeroed".
 ...
 * @return Returns the real portion (X) of the quaternion.
 */
float AHRS::GetQuaternionZ() { return quaternionZ; }
...
void AHRS::UpdateDisplacement(float accel_x_g, float accel_y_g, int update_rate_hz, bool is_moving)
{
 ...
 * @return Current Velocity (in meters/squared).
 */
float AHRS::GetVelocityZ() { return (ahrs_internal->IsDisplacementSupported() ? velocity[2] : 0.f); }
 ...
 * @return Displacement since last reset (in meters).
 */
float AHRS::GetDisplacementZ() { return (ahrs_internal->IsDisplacementSupported() ? displacement[2] : 0.f); }
...
void AHRS::commonInit(uint8_t update_rate_hz)
{
    ...
    yaw = pitch = roll = compass_heading = 0.0f;
    world_linear_accel_x = world_linear_accel_y = world_linear_accel_z = 0.0f;
    mpu_temp_c = 0.0f;
    fused_heading = 0.0f;
    altitude = 0.0f;
    baro_pressure = 0.0f;
    ...
    quaternionW = quaternionX = quaternionY = quaternionZ = 0.0f;
    ...
    /* Raw Data */
    raw_gyro_x = raw_gyro_y = raw_gyro_z = 0.0f;
    raw_accel_x = raw_accel_y = raw_accel_z = 0.0f;
    cal_mag_x = cal_mag_y = cal_mag_z = 0.0f;
    ...
    capability_flags = 0;
    op_status = sensor_status = cal_status = selftest_status = 0;
    /* Board ID */
    board_type = hw_rev = fw_ver_major = fw_ver_minor = 0;
...
double AHRS::GetAngle() { return yaw_angle_tracker->GetAngle(); }
...
double AHRS::GetRate() { return yaw_angle_tracker->GetRate(); }
 ...
 */
float AHRS::GetRawGyroX() { return this->raw_gyro_x / (DEV_UNITS_MAX / (float)gyro_fsr_dps); }
 ...
 * @return Returns the current rotation rate (in degrees/sec).
 */
float AHRS::GetRawGyroY() { return this->raw_gyro_y / (DEV_UNITS_MAX / (float)gyro_fsr_dps); }
 ...
 * @return Returns the current rotation rate (in degrees/sec).
 */
float AHRS::GetRawGyroZ() { return this->raw_gyro_z / (DEV_UNITS_MAX / (float)gyro_fsr_dps); }
 ...
 */
float AHRS::GetRawAccelX() { return this->raw_accel_x / (DEV_UNITS_MAX / (float)accel_fsr_g); }
 ...
 * @return Returns the current acceleration rate (in G).
 */
float AHRS::GetRawAccelY() { return this->raw_accel_y / (DEV_UNITS_MAX / (float)accel_fsr_g); }
 ...
 * @return Returns the current acceleration rate (in G).
 */
float AHRS::GetRawAccelZ() { return this->raw_accel_z / (DEV_UNITS_MAX / (float)accel_fsr_g); }
 ...
 */
float AHRS::GetRawMagX() { return this->cal_mag_x / UTESLA_PER_DEV_UNIT; }
 ...
 * @return Returns the mag field strength (in uTesla).
 */
float AHRS::GetRawMagY() { return this->cal_mag_y / UTESLA_PER_DEV_UNIT; }
 ...
 * @return Returns the mag field strength (in uTesla).
 */
float AHRS::GetRawMagZ() { return this->cal_mag_z / UTESLA_PER_DEV_UNIT; }
 ...
 */
float AHRS::GetTempC() { return this->mpu_temp_c; }
...
std::string AHRS::GetSmartDashboardType() const { return "Gyro"; }
 ...
 */
double AHRS::PIDGet() { return GetYaw(); }
...
#define NAVX_MOTION_PROCESSOR_UPDATE_RATE_HZ 200
    ...
    int realized_update_rate
        = NAVX_MOTION_PROCESSOR_UPDATE_RATE_HZ / (NAVX_MOTION_PROCESSOR_UPDATE_RATE_HZ / integer_update_rate);
...
int AHRS::GetRequestedUpdateRate() { return (int)update_rate_hz; }

void AHRS::Close() { io->Stop(); }
