...
SerialIO::SerialIO(std::string port_id, uint8_t update_rate_hz, bool processed_data,
    IIOCompleteNotification* notify_sink, IBoardCapabilities* board_capabilities)
{
...
int SerialIO::DecodePacketHandler(char* received_data, int bytes_remaining)
{
    ...
    if ((packet_length = IMUProtocol::decodeYPRUpdate(received_data, bytes_remaining, ypr_update_data)) > 0) {
        ...
        // printf("UPDATING YPR Data\n");
                   ...
                   = AHRSProtocol::decodeAHRSPosTSUpdate(received_data, bytes_remaining, ahrspos_ts_update_data))
        > 0) {
...
void SerialIO::Run()
{
    ...
    try {
        ...
        cmd_packet_length = AHRSProtocol::encodeDataGetRequest(
            stream_command, AHRS_DATA_TYPE::BOARD_IDENTITY, AHRS_TUNING_VAR_ID::UNSPECIFIED);
        serial_port->Write(stream_command, cmd_packet_length);
        serial_port->Flush();
        port_reset_count++;
    ...
    while (!stop) {
        try {
            ...
            if (signal_transmit_integration_control) {
                ...
                cmd_packet_length
                    = AHRSProtocol::encodeIntegrationControlCmd(integration_control_command, integration_control);
                try {
            ...
            if (bytes_read > 0) {
                ...
                while (i < bytes_read) {
                    ...
                    if (received_data[i] != PACKET_START_CHAR) {
                        ...
                        if ((bytes_remaining > 2) && (received_data[i + 1] == BINARY_PACKET_INDICATOR_CHAR)) {
                            ...
                            while (bytes_remaining < total_expected_binary_data_bytes) {
                                ...
                                if (additional_received_data_length > 0) {
                                    memcpy(received_data + bytes_remaining, additional_received_data,
                            ...
                            if (packet_length > 0) {
                                ...
                                if (partial_packet) {
                                    if (bytes_remaining > (int)sizeof(remainder_data)) {
                                        memcpy(remainder_data, received_data + i - sizeof(remainder_data),
                ...
                // std::cout << "Config: " << retransmit_stream_config << " Time: " << (time(0) -
                // last_stream_command_sent_timestamp ) << " " << last_stream_command_sent_timestamp << std::endl;

                // DEBUG-TODO: Enable this if we really need it
                if (false
                    && (retransmit_stream_config
...
double SerialIO::GetByteCount() { return byte_count; }

double SerialIO::GetUpdateCount() { return update_count; }

void SerialIO::SetUpdateRateHz(uint8_t update_rate) { update_rate_hz = update_rate; }

void SerialIO::ZeroYaw() { EnqueueIntegrationControlMessage(NAVX_INTEGRATION_CTL_RESET_YAW); }

void SerialIO::ZeroDisplacement()
{
    EnqueueIntegrationControlMessage(
        NAVX_INTEGRATION_CTL_RESET_DISP_X | NAVX_INTEGRATION_CTL_RESET_DISP_Y | NAVX_INTEGRATION_CTL_RESET_DISP_Z);
}

void SerialIO::Stop() { stop = true; }
